  ...
  if (license.isExpired) {
    return (
      <div className="bg-red-600 text-white text-center text-xs font-medium py-1.5 px-4 flex items-center justify-center gap-2">
        ...
        <span>ライセンスの有効期限が切れています。データの閲覧は可能ですが、登録・編集・削除は制限されています。</span>
      </div>
    );
  }

  if (license.isTrial && license.daysRemaining !== null && license.daysRemaining <= 14) {
    return (
      <div className="bg-yellow-500 text-white text-center text-xs font-medium py-1.5 px-4 flex items-center justify-center gap-2">
        ...
        <span>トライアル期間の残りが{license.daysRemaining}日です。引き続きご利用いただくには有料プランへの切り替えが必要です。</span>
        <button onClick={() => setDismissed(true)} className="ml-2 hover:opacity-80" aria-label="閉じる">
